...
namespace Nop.Web.Extensions.Bootstrap4
{
    ...
    /// </summary>
    [HtmlTargetElement("nop-bootstrap-date-picker",
        Attributes = DAY_NAME_ATTRIBUTE_NAME + "," + MONTH_NAME_ATTRIBUTE_NAME + "," + YEAR_NAME_ATTRIBUTE_NAME,
        TagStructure = TagStructure.WithoutEndTag)]
    public class NopBootstrapDatePickerTagHelper : TagHelper
    {
        ...
        public NopBootstrapDatePickerTagHelper(IHtmlGenerator generator, IHtmlHelper htmlHelper)
        {
            Generator = generator;
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            daysList.AddCssClass("form-control");
            monthsList.AddCssClass("form-control");
            yearsList.AddCssClass("form-control");
            ...
            days.AppendFormat("<option value='{0}'>{1}</option>", "0", await locService.GetResourceAsync("Common.Day"));
            for (var i = 1; i <= 31; i++)
                days.AppendFormat("<option value='{0}'{1}>{0}</option>", i,
                    (SelectedDay.HasValue && SelectedDay.Value == i) ? " selected=\"selected\"" : null);

            months.AppendFormat("<option value='{0}'>{1}</option>", "0", await locService.GetResourceAsync("Common.Month"));
            for (var i = 1; i <= 12; i++)
            {
            ...
            years.AppendFormat("<option value='{0}'>{1}</option>", "0", await locService.GetResourceAsync("Common.Year"));
            ...
            var wrapDaysList = WrapInDivWithClass(WrapInDivWithClass(await daysList.RenderHtmlContentAsync(), "input-group"), "col-auto form-group");
            var wrapMonthsList = WrapInDivWithClass(WrapInDivWithClass(await monthsList.RenderHtmlContentAsync(), "input-group"), "col-auto form-group");
            var wrapYearsList = WrapInDivWithClass(WrapInDivWithClass(await yearsList.RenderHtmlContentAsync(), "input-group"), "col-auto form-group");
        ...
        private static string WrapInDivWithClass(string innerText, string className)
        {
            return string.Format($"<div class=\"{ className }\">{ innerText }</div>");
        }
    }
}
